...
from AyiinOXD.ayiin import ayiin_cmd, eod, eor
...
async def sketch(e):
    ...
    cv2.imwrite("AyiinOXD.png", pencil_sketch_IMG)
    await xx.respond(get_string("yimg_3"), file="AyiinOXD.png")
    await xx.delete()
    os.remove(file)
    os.remove("AyiinOXD.png")
...
async def xnxx(event):
    ...
    if ayiin.endswith(".tgs"):
        ...
        file = "yin.png"
    yin = cv2.imread(file)
    AyiinOXD= cv2.cvtColor(yin, cv2.COLOR_BGR2GRAY)
    cv2.imwrite("yin.jpg", AyiinOXD)
...
async def ayiin(event):
    ...
    if yins.endswith(".tgs"):
        ...
        file = "yin.png"
    yin = cv2.imread(file)
    AyiinOXD= cv2.GaussianBlur(yin, (35, 35), 0)
    cv2.imwrite("yin.jpg", AyiinOXD)
    await event.client.send_file(
        event.chat_id,
        "yin.jpg",
...
async def yinsxd(event):
    ...
    AyiinOXD= await ureply.download_media()
    if AyiinOXD.endswith(".tgs"):
        await ayiin.edit(get_string("com_1"))
        cmd = ["lottie_convert.py", AyiinOXD, "yin.png"]
        file = "yin.png"
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
...
async def ayiin(kontol):
    ...
    if AyiinOXD.endswith(".tgs"):
        ...
        cmd = ["lottie_convert.py", AyiinOXD, "yins.png"]
        file = "yins.png"
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
...
async def ayiin(memek):
    ...
    AyiinOXD= cv2.vconcat([mici, trn])
    cv2.imwrite("yins.jpg", AyiinOXD)
...
async def yins(event):
    ...
    AyiinOXD= asu.reshape(yins.shape)
    cv2.imwrite("yins.jpg", AyiinOXD)
    await event.client.send_file(
        event.chat_id,
        "yins.jpg",
