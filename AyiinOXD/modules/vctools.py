...
from AyiinOXDimport CMD_HELP, bot
from AyiinOXD.ayiin import ayiin_cmd, eod, eor
from AyiinOXD.events import register
from AyiinOXD.ayiin.pytgcalls import Ayiin, CLIENTS, VIDEO_ON
...
async def _(event):
    ...
    if sender.id != yins.id:
        AyiinOXD= await event.reply(get_string("com_1"))
    else: 
        AyiinOXD= await eor(event, get_string("com_1"))
    if len(event.text.split()) > 1:
        chat = event.text.split()[1]
        try:
            chat = await event.client.parse_id(chat)
        except Exception as e:
            return await eod(AyiinOXD, get_string("error_1").format(str(e)))
    else:
        chat = event.chat_id
    Xd = Ayiin(chat)
    if not Xd.group_call.is_connected:
        await Xd.group_call.join(chat)
        await AyiinOXD.edit(get_string("jovc_1").format(yins.first_name, yins.id, chat)
        )
...
async def _(event):
    ...
    if VIDEO_ON.get(chat):
        del VIDEO_ON[chat]
    await AyiinOXD.edit(get_string("levc_1").format(yins.first_name, yins.id, chat))
