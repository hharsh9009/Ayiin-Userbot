...
from AyiinOXDimport CMD_HELP, DEVS, LOGS, STORAGE
...
async def impostor(event):
    ...
    AyiinOXD= ["@AyiinOXD", "@AyiinOXD"]
    if inputArgs in AyiinOXD:
        await eor(event, get_string("ayiin_2"))
        await event.client.send_message("@AyiinOXD", get_string("ayiin_3")
                                        )
        return
