...
async def testing(AyiinOXD):
    ayiin = await AyiinOXD.get_chat()
    yins = await AyiinOXD.client.get_me()
    admin = ayiin.admin_rights
    creator = ayiin.creator
    if not admin and not creator:
        await eod(AyiinOXD, get_string("stvc_1").format(yins.first_name))
        return
    xnxx = await eor(AyiinOXD, get_string("yiban_1"))
# Thank for Dark_Cobra
    ayiinkontol = await AyiinOXD.client.get_participants(AyiinOXD.chat_id)
    for user in ayiinkontol:
        if user.id == yins.id:
            pass
        try:
            xx = await AyiinOXD.client(EditBannedRequest(AyiinOXD.chat_id, int(user.id), ChatBannedRights(until_date=None, view_messages=True)))
